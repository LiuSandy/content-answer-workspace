...
from app.application.agent import security
from app.application.agent.tools import ALL_TOOLS
from app.infrastructure.knowledge.ssrf import SSRFError
...
def test_zhihu_search_is_builtin_without_agent_reach_config(monkeypatch, tmp_path):
    from app.application.agent import tools
...
async def test_web_fetch_tool_refuses_internal_url_without_network():
    ...
    from app.application.agent.tools.web_fetch import web_fetch
...
async def test_fetch_web_page_blocks_redirect_hop_to_internal(monkeypatch):
    ...
    from app.infrastructure.knowledge import ssrf as ssrf_mod
