...
async def test_evaluate_top_n_scores_opportunities(monkeypatch):
    ...
    llm = _mock_llm()
    monkeypatch.setattr(
        "app.modules.acquisition.application.topic_analyst.get_acquisition_llm", lambda: llm
    )
    monkeypatch.setattr(
        "app.modules.acquisition.application.topic_analyst.TopicAnalystService._get_active_memories",
