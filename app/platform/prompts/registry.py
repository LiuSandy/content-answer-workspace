...
class PromptRegistry:
    ...
    def render(self, prompt_id: str, **variables: Any) -> RenderedPrompt:
        ...
        # 解析模型参数
        provider, model_str, temperature, max_tokens, structured_methods = (
            self._resolve_model_params(schema)
        )
    ...
    def _resolve_model_params(
    ...
    ) -> tuple[str | None, str | None, float, int, list[str]]:
        ...
        return provider, model_str, temperature, max_tokens, structured_methods
...
def warmup(prompts_dir: Path | None = None, freeze: bool = True) -> None:
    ...
    if prompts_dir is None:
        # v3 prompts 分布于各业务模块和共享工具包，统一从 app 根目录发现。
