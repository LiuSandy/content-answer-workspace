...
async def run_memory_extraction(
    ...
    """Compatibility entrypoint delegating production work to the Memory use case.

    Explicit ``llm``/``embedding_provider`` arguments retain the legacy seam for
    existing isolated tests while runtime callers use the injected v3 slice.
    """
    if llm is None and embedding_provider is None:
        from app.bootstrap.container import get_memory_extraction_use_case

        outcome = await get_memory_extraction_use_case().execute(
