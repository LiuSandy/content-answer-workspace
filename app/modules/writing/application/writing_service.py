...
from app.modules.documents.adapters.db.models import AIOperation, AnswerVersion
from app.platform.prompts.registry import prompt_registry, RenderedPrompt
from app.shared.llm.dto import LLMMessage as GatewayMessage, LLMRequest as GatewayRequest
from app.shared.llm.port import LLMGatewayPort

logger = logging.getLogger(__name__)


def _get_llm_gateway() -> LLMGatewayPort:
...
async def run_writer_stream(
    ...
    *,
    ...
    try:
        llm_req = GatewayRequest(
            messages=[GatewayMessage.model_validate(m.model_dump()) for m in rendered.messages],
            provider=rendered.provider,
            model=rendered.model,
            temperature=rendered.temperature,
            max_tokens=rendered.max_tokens,
        )
        async for event in gateway.stream(purpose=f"writing.{operation}", request=llm_req):
            if event.delta:
        ...
        if defer_version:
            ...
            capture.provider = rendered.provider
