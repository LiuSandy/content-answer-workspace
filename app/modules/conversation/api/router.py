...
from app.modules.conversation.application.run_context import branch_thread_id, compose_run_inputs
from app.modules.conversation.application.summary_updater import SummaryUpdater
from app.modules.memory.application.extraction_compat import run_memory_extraction
...
from app.platform.config.runtime import AGENT_MAX_RECURSION, AGENT_RUN_TIMEOUT
        ...
        try:
            ...
            async for name, data in run_agent_stream(graph, inputs, config, timeout_seconds=AGENT_RUN_TIMEOUT):
                if name in ("agent.status", "tool.started"):
                    ...
                    from app.platform.config.runtime import is_rag_source_display_enabled
