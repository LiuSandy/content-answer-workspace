...
def build_writer_dispatch_node(writer_graph: Any | None = None):
    async def writer_dispatch_node(state: ChatAgentState) -> dict:
        graph = writer_graph
        if graph is None:
            from app.modules.writing.agent.graph import writer_graph as default_writer_graph
        ...
        try:
            ...
            if intent == "task_plan":
                ...
                try:
                    from app.modules.writing.agent.nodes.task_plan_persistence import persist_task_plan
