...
const sourceColor = (src: string) => {
  ...
  if (src === "vector") return "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300";
  if (src === "reranked") return "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300";
...
export const RetrievalTraceDialog: React.FC<RetrievalTraceDialogProps> = ({
  ...
  const { data: fetchedTrace, isLoading, error } = useQuery({
  ...
  const trace: RetrievalTrace | undefined = fetchedTrace ?? (traceData as RetrievalTrace | undefined);
        ...
        {trace && (
          <div className="space-y-4 text-xs">
            ...
            <div className="grid grid-cols-3 gap-2 p-3 bg-muted/40 rounded-md">
              <div>
                <div className="text-[9px] text-muted-foreground uppercase tracking-wide mb-0.5">RAG 决策</div>
                <Badge variant={trace.ragDecision ? "default" : "secondary"} className="text-[10px]">
                  {trace.ragDecision ? "需要检索" : "跳过检索"}
                </Badge>
              </div>
              <div>
                <div className="text-[9px] text-muted-foreground uppercase tracking-wide mb-0.5">检索模式</div>
                <Badge variant="outline" className="text-[10px]">{trace.mode}</Badge>
              </div>
              {trace.latencyMs != null && (
                <div>
                  <div className="text-[9px] text-muted-foreground uppercase tracking-wide mb-0.5">耗时</div>
                  <span className="font-mono font-semibold">{trace.latencyMs}ms</span>
                </div>
              )}
              {trace.embeddingModel && (
                <div className="col-span-2">
                  <div className="text-[9px] text-muted-foreground uppercase tracking-wide mb-0.5">Embedding</div>
                  <span className="font-mono text-[10px]">{trace.embeddingModel}</span>
                </div>
              )}
              {trace.rerankerModel && (
                <div>
                  <div className="text-[9px] text-muted-foreground uppercase tracking-wide mb-0.5">Reranker</div>
            ...
            <div className="space-y-1.5">
              <div className="text-[9px] text-muted-foreground uppercase tracking-wide">原始查询</div>
              <div className="p-2 bg-muted/30 rounded">{trace.originalQuery}</div>
              {trace.rewrittenQuery && (
                <>
                  <div className="text-[9px] text-muted-foreground uppercase tracking-wide mt-2">改写查询</div>
                  <div className="p-2 bg-primary/5 border border-primary/20 rounded">{trace.rewrittenQuery}</div>
                </>
              )}
            </div>
            ...
            {trace.decisionReason && (
              <div>
                <div className="text-[9px] text-muted-foreground uppercase tracking-wide mb-1">决策原因</div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {trace.hits.map((hit, idx) => (
                    ...
                    >
                      <span className={`shrink-0 px-1.5 py-0.5 rounded text-[9px] font-medium ${sourceColor(hit.retrievalSource)}`}>
                        {hit.retrievalSource}
                      </span>
                      <span className="font-mono text-[9px] text-muted-foreground">#{hit.rank}</span>
                      {hit.citationLabel && (
                        <Badge variant="outline" className="text-[9px] px-1 py-0 h-4">{hit.citationLabel}</Badge>
                      )}
