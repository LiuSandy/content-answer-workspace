...
export function getChatMarkdownComponents(isUser: boolean) {
  return {
    ...
    blockquote: ({ node, ...props }: any) => (
      <blockquote className={cn("border-l-4 pl-3 my-2 italic", isUser ? "border-primary-foreground/30 text-primary-foreground/75" : "border-muted-foreground/30 text-muted-foreground")} {...props} />
    ),
    code: ({ node, inline, className, children, ...props }: any) => !inline ? (
      ...
      <code className={cn("px-1 py-0.5 rounded text-xs font-mono border", isUser ? "bg-primary-foreground/10 text-primary-foreground border-primary-foreground/20" : "bg-muted text-foreground border-border/30")} {...props}>{children}</code>
    ),
    ...
    a: ({ node, ...props }: any) => (
      <a className={cn("hover:underline break-all font-medium", isUser ? "text-primary-foreground" : "text-blue-500")} target="_blank" rel="noreferrer" {...props} />
    ),
  };
}
