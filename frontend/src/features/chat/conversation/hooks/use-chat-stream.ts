...
export function useChatStream(onStreamStart: () => void) {
  ...
  const { currentChatId, activeLeafMessageId, setCurrentChatId, setActiveLeafMessageId } =
    useChatStore();
  ...
  const refreshAfterStream = useCallback(
    async (chatId: string, controller: AbortController, refreshFallback = false) => {
      ...
      try {
        ...
        streamingController.settle(reconcileTransientStreamError(transientError, updatedMessages));
  ...
  const startStream = useCallback(
    (status: string, clearWorkspace = true) => {
  ...
  const sendMessage = useCallback(
    async (content: string, parentId?: string | null) => {
      ...
      if (!activeChatId) {
        try {
          const newChat = await apiPost<{ chatId: string; title: string }>("/api/chats", {
            title: "新对话",
          });
  ...
  const selectChoice = useCallback(
    async (optionId: string, messageId: string) => {
      ...
      try {
        await streamPost(
          `/api/chats/${currentChatId}/choices`,
          {
            messageId,
            selection: optionId,
          },
    ...
    [currentChatId, handleStreamEvent, refreshAfterStream, startStream, streamingController],
  );
