...
1. 后端日志同时输出到控制台和本地文件。
2. 控制台使用紧凑的人类可读格式，日志文件使用单行 JSON 格式。
...
控制台输出用于人工实时阅读，不使用 JSON。基础格式如下：

```text
23:08:02 INFO     app.infrastructure.scheduler           | APScheduler started
23:10:25 INFO     app.api.routes.chats                   | Chat started  request_id=req_123 run_id=run_456 chat_id=chat_789
```
...
- 时间只显示本地时间 `HH:MM:SS`。
- 显示日志等级、logger 名称和消息。
- 只追加有值的上下文字段，不显示 `null` 字段。
- 终端支持颜色时对时间和日志等级着色；重定向输出时不写 ANSI 颜色码。
- 异常堆栈按正常多行文本显示。
- 单个复杂字段在控制台最多显示 200 个字符，完整值仍保存在 JSON 文件中。

### 5.3 文件 JSON 格式

每条文件日志占一行，不允许跨行写入非 JSON 内容。基础结构如下：
...
4. 控制台使用紧凑文本格式，不显示 JSON 大对象和空值上下文字段。
5. 所有 `.log` 文件中的每一行都能被 JSON 解析。
6. 日志按照 `logs/YYYY-MM-DD/<level>.log` 正确分类，不产生汇总日志文件。
7. 跨天后自动写入新的日期目录，超过大小上限后只轮转对应级别文件。
8. 超过保留期的历史日期目录可以按配置整体清理。
9. HTTP 响应包含 `X-Request-ID`，同一请求日志中的 request ID 一致。
10. 并发请求之间的上下文不会串线。
11. SSE 流结束前保留正确的 request ID 和 run ID。
12. PDF 识别日志包含 job ID，单页日志包含 page number。
13. 对话和生成日志包含对应的 run ID。
14. Token、Cookie、密码、API Key 和签名不会以明文出现在控制台或日志文件中。
15. 应用日志、Uvicorn 日志和第三方日志不会重复输出。
16. 日志写入不会明显阻塞对话、SSE 或 PDF 后台识别任务。
